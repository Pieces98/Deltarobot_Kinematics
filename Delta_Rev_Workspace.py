...
deg1 = np.deg2rad(np.linspace(deg_min, deg_max, size))
deg2 = np.deg2rad(np.linspace(deg_min, deg_max, size))
deg3 = np.deg2rad(np.linspace(deg_min, deg_max, size))

print("Create Angle Vector")
deg_Aleg = np.array([[deg1[idx], deg2[jdx], deg3[kdx]] for idx in range(size) for jdx in range(size) for kdx in range(size)]).T

print("Angle Vector Size : ", deg_Aleg.shape)                                
...
ax.scatter(res_coor[0,:], res_coor[1,:], res_coor[2,:], s=1.5, c=res_coor[2,:], cmap='jet')
ax.set_xlabel('X')
ax.set_ylabel('Y')
ax.set_zlabel('Z')
plt.show(block=True)
